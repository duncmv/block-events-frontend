...
const EventCard = ({ event }: EventCardProps) => {
  ...
  }, [isAuthenticated, currentUser, event._id]);

  const medialUrl = 'http://localhost:3300/media/';
  const pics = <img src={medialUrl + event.media} alt="" className="w-full h-full object-cover" />;

  return (
    <div className="max-w-xs w-full h-[24rem] lg:h-[28rem] rounded-lg overflow-hidden shadow-lg bg-white hover:shadow-xl transition-shadow duration-300">
      <a
        onClick={() => {
